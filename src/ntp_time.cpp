...
void ntp_init() {
    ...
    Serial.println("[NTP] Synchronizuję czas...");
    ...
    while (!getLocalTime(&timeinfo, 1000)) {
        if (millis() - start > NTP_TIMEOUT_MS) {
            Serial.println("[NTP] ✗ Timeout — brak sync");
            return;
        }
        Serial.print(".");
    }
    ...
    Serial.printf("[NTP] ✓ Czas: %04d-%02d-%02d %02d:%02d:%02d\n",
...
void ntp_tick() {
    ...
    if (_last_sync_ms == 0 || (now - _last_sync_ms >= NTP_RESYNC_INTERVAL_MS)) {
        ...
        if (getLocalTime(&timeinfo, 5000)) {
            _synced      = true;
            _last_sync_ms = now;
            Serial.printf("[NTP] ✓ Resync: %04d-%02d-%02d %02d:%02d:%02d UTC\n",
                timeinfo.tm_year + 1900, timeinfo.tm_mon + 1, timeinfo.tm_mday,
                timeinfo.tm_hour, timeinfo.tm_min, timeinfo.tm_sec);
        } else {
            Serial.println("[NTP] ✗ Resync nieudany");
        }
    }
}